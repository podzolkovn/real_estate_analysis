...
from app.infrastructure.db.models import InflationModel
...
class InflationModelRepository(SqlAlchemyBaseRepository):
    def __init__(self, session: Annotated[AsyncSession, Depends(get_async_session)]):
        super().__init__(model=InflationModel, session=session)
